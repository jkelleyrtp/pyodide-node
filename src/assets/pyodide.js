...
var languagePluginLoader = new Promise((resolve, reject) => {
  ...
  let _loadPackage = (names, messageCallback, errorCallback) => {
    ...
    let packages = self.pyodide.packages.dependencies;
    ...
    let promise = new Promise((resolve, reject) => {
      ...
      self.pyodide.monitorRunDependencies = () => {
        packageCounter--;
        if (packageCounter === 0) {
          for (let pkg in toLoad) {
            self.pyodide.loadedPackages[pkg] = toLoad[pkg];
          }
          delete self.pyodide.monitorRunDependencies;
  ...
  Module.autocomplete = function (path) {
    ...
    return pyodideget_completions(path);
  };
